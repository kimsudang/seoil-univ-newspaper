...
public class PopupUploadController implements Controller {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!isMultipart) {
			...
			try {
				...
				result = service.popInsert(pop_title, pop_img_path, pop_url, start_date, end_date, pop_loc_top, pop_loc_left);
				...
				result = service.popImageInsert(pop_img_path,keyList.getPop_no(),pReg_date, newFilePath);
				if(result <= 0) {
					File fileremove = new File(request.getSession().getServletContext().getRealPath("/")+pop_img_path);
					fileremove.delete();
					request.setAttribute("result",result);	
